...
/*{
  ...
  content: (<div>
    ...
    <p>This control involves securely generating, storing, and analyzing logs generated by the devices in your environment.</p>

    <h3>Centralized Log Management</h3>
    <p>Utilize a centralized log management system which will collect logs from the devices in your environment. This centralized location is beneficial not only because it provides you with a single location to review logs, but it is also makes it far more difficult for adversaries to cover their tracks.</p>
    <p>Some things to keep in mind are to make sure there is enough space to store all of the logs generated, for the period of time your organization determines is appropriate. You also want to utilize three synchronized time sources to ensure timestamps are consistent in your logs. </p>

    <h3>Detailed Audit Logging</h3>
    <p>Now that you have set up a centralized logging system to aggregate the logs generated in your environment, it is time to configure your devices to generate detailed logs and send them to your central logging system. A good approach here is to generate lots of detailed logs on the endpoint, and then determine which of those logs should be sent to the central logging server. Choosing what logs are important to collect will help reduce bandwidth in the environment and storage on the logging system.</p>
    <p>Some detailed logging items include:</p>
    <ul>
      <li>Timestamp</li>
      <li>Source IPs</li>
      <li>Destination IPs</li>
      <li>Devices</li>
      <li>Event Source</li>
      <li>User</li>
      <li>Command/Service/Application Name</li>
    </ul>

    <h3>Analyzing Logs</h3>
    <p>Now that you are generating logs and sending them to a centralized location, it is time to implement a strategy for analyzing those logs. This strategy involves both utilizing software to analyze logs and manual review of logs. Log analytic tools have come a long way, but they are not foolproof. They should be treated as a tool to help the analyst work through the mountain of generated logs.</p>
